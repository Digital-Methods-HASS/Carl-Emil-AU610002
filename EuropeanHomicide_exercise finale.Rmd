...
```{r plot-homicides, eval = FALSE}
 ggplot(data = Western_Europe, aes(x = Year, y = homicides_per_100k, color = Entity)) + 
  geom_line() +
  labs(title = "Long-term Trend in Homicides per 100k in Western Europe 1300 - 2016",
       x = "Year",
...
```{r eval = FALSE}
## This part from here on is the Monarchs exercise:
monarchs <- read.csv("data/Kongerækken).csv")
...
```{r eval = FALSE}
monarchs %>% 
  mutate(duration = Start_of_reign - End_of_reign, midyear = End_of_reign - duration/2) %>% 
...
"Not to be counted in the word count, this is just information before the answer: This answer is also given in the word document that is submitted in the final portofolio as my exam".
Answwr to question 3:
I can conclude the following to the asked question with help from the given data. The plots provided to me through the data and with the help of Rstudio allows me to gain a visualization of the trend in Homicides in Europe throughout the time from 1306 to 2016. As we could have hoped for our civilizations the homicide rates appear to be descending over time. The data also tells us, where the homicide rates has been at it highest and lowest throughout history. Looking at a country as Italy for example, we can see that it had the highest tendency in Homicides of all times out of the researched countries. However, the rate of homicides declines in all the given countries over time, so that is of cause a positive outcome. This also helps answering the question “are we more civilized today?”. Yes, if we go by homicide rate, we are more civilized today, and we have evolved from that need of killing. Murderers still exist in our society today, but there are far fewer than earlier in our history. We can put this to further discussion and wonder, what we actually mean by being civilized, but for the sake of argument, that the lowered rate in Homicides in Europe, has led us to become more civilized, I will not enter into this discussion at this point. The homicide rate is just one of many indicators that has led us to the kind of society that we are today. 
